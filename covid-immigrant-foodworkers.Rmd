...
ipums_food_production_employment_by_ind_occ <- ipums_food_production %>% 
...
# Export the data
write_csv(ipums_food_production_employment_by_ind_occ, "data/exported/ipums_food_production_employment_by_ind_occ.csv")
```

```{r filter the industry-occupation pairs}
# Import data frame of food production industry-occupation pairs
ipums_food_production_employment_by_ind_occ <- read_csv("data/ipums_food_production_employment_by_ind_occ.csv")
...
ipums_food_production_selected_occupations <- ipums_food_production %>% 
  inner_join(ipums_food_production_employment_by_ind_occ, by = c("industry", "occupation")) %>% 
...
### Analyze the data nationally

```{r analyze the data by total foreign}
...
```{r analyze by foreign and Hispanic}
...
ipums_food_production_by_puma %>% 
  ...
  head(23) %>% 
  ungroup() %>% 
  summarize(sum(total_foreign_hispanic)) # 2,341 PUMAs / 100 = 23.41
```
...
### THE REST OF THIS CODE IS NOT READY FOR PRIME TIME
...
# And join to the population and COVID data
ipums_food_production_by_county <- list(ipums_food_production_by_county, covid, population) %>% 
  reduce(left_join, by = "county_code") %>% 
  mutate(cases_per_100k = round(cases / population * 100000, digits = 0),
         deaths_per_100k = round(deaths / population * 100000, digits = 0)) %>% 
